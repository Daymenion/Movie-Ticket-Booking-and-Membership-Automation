...
namespace BookMyshow
{
    public partial class BookM : Form
    {
        ...
        private void comboBox2_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                names = ds.Tables[0].Rows[comboBox3.SelectedIndex][0].ToString();
                dates = (DateTime)ds1.Tables[0].Rows[comboBox2.SelectedIndex][0];
                da.SelectCommand.Parameters.Clear();
                Console.WriteLine(dates);
                da.SelectCommand = new SqlCommand("select DISTINCT tname from mtable1 where date='" + dates.ToString("yyyy-MM-dd") + "' and mname='" + names + "' ", cs);
                ds2.Clear();
                da.Fill(ds2);

                tname = ds2.Tables[0].Rows[0][0].ToString();
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                da.SelectCommand = new SqlCommand("select TIME,ts_id from mtable1 where date='" + dates.ToString("yyyy-MM-dd") + "' and mname='" + names + "' and tname='" + tname + "'", cs);
                ds3.Clear();
                da.Fill(ds3);
